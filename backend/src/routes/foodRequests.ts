...
foodRequestsRouter.post('/:id/pledges', requireAuth, validateBody(z.object({
  ...
  try {
    ...
    const { rows: reqRows } = await withClient((client) => client.query('SELECT requester_name, email, quantity AS requested_quantity, requested_item FROM food_requests WHERE id = $1', [id]));
